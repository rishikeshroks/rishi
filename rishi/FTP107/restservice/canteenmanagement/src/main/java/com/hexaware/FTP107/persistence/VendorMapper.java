...
public class VendorMapper implements ResultSetMapper<Vendor> {
  ...
  public final Vendor map(final int idx, final ResultSet rs, final StatementContext ctx) throws SQLException {
       ...
       * @ return vid
       */
    return new Vendor(rs.getInt("V_ID"), rs.getString("V_NAME"), rs.getString("V_USERNAME"),
                      rs.getString("V_PASSWORD"), rs.getString("V_EMAIL"), rs.getString("V_PHONE"), rs.getString("V_ADDRESS"));
  }
}
