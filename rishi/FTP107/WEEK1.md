...
IdentityFile ~/.ssh/id_rsa
  ...
  ** Clone repository at Gitbash **
    ...
    * `pwd` -- Check and verify that the current working directory is C:\users\Hvuser/workspace
    * `git clone ssh://git-codecommit.us-east-2.amazonaws.com/v1/repos/FTP107 `
    * `cd FTP107`
    * `git status`
  * open Visual Studio (VS) Code
    * Open folder c:\users\Hvuser\workspace\FTP107
...
# Workshop material - Day #2 - MySQL Workbench/Cli/Curl/Browser to interact w/ application
  ...
  * `CREATE DATABASE FTP107;` and click the lightning button
  * `CREATE USER 'FTP107'@'localhost' IDENTIFIED BY 'FTP107';`
  * `GRANT ALL ON FTP107.* TO 'FTP107'@'localhost';`
  ...
  * go to gitbash, ensure you are in workspace/FTP107
  * `cd restservice/canteenmanagement`
  * `mvn compile`
  * `mvn exec:java -Dexec.mainClass=com.hexaware.FTP107.util.CliMain`
  ...
  * Build the war file as follows:
    ...
    * `cp target/FTP107-0.0.1-SNAPSHOT.war /D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16/webapps/FTP107.war`
    * start tomcat and tail its logs
      * `cd D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16`
      * `rm -rf logs/*`
      * `./bin/startup.sh`
      * `tail -f logs/*`
      * `curl -vvv http://localhost:8080/FTP107/api/menu`
      * `cd ../../webui/lmApp/`
   * `npm install`
   * `ng serve`
...
# Workshop material - Day #4 - Configure Project Board in Trello for  Managing and Tracking Your Project
  ...
  * Create a board for your project as "FTP107"
  ...
  * [Team member #3]
     * Create "Current Sprint" with "FTP107 Sprint 1" as label 
